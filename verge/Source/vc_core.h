...
public:
	...
	void LoadCore(EMUFILE *f, int cimage, bool append = false, bool patch_others = false);
	void UnloadCore(int cimage);
	Chunk *GetCore(int cimage);

	void LoadMapScript(EMUFILE *f, CStringRef filename);
	void ExecAutoexec();
	void Decompile();
...
public:
	...
	typedef std::map<std::string,ScriptBlob*> TScriptMap;
	TScriptMap scripts;
	void Save(EMUFILE* f);
	void Load(EMUFILE* f);
};

extern ScriptDatabase g_ScriptDatabase;