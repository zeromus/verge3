...
ScriptDatabase g_ScriptDatabase;
...
void VCCore::LoadCore(EMUFILE *f, int cimage, bool append, bool patch_others)
{
	assert(!append);
	...
	/*if(append)
		coreimages[cimage].Append(f, true);
	else {*/
		new(&coreimages[cimage])Chunk(); //inplace construct a new chunk to make sure everything is zapped
		coreimages[cimage].LoadChunk(f);
	//}
}

void VCCore::LoadMapScript(EMUFILE *f, CStringRef filename)
{
...
void ScriptDatabase::Save(EMUFILE* f)
{
	f->write32le(scripts.size());
	for(TScriptMap::iterator it(scripts.begin()); it != scripts.end(); ++it)
	{
		//write script id
		const std::string& id = it->first;
		f->write32le(id.size());
		f->fwrite(&id[0],id.size());
		f->fputc(0);
		//write script blob size
		f->write32le(it->second->data.size());
		//write script blob
		f->fwrite(&it->second->data[0],it->second->data.size());
	}
}
void ScriptDatabase::Load(EMUFILE* f)
{
	quad numscripts = f->read32le();
	for(quad i=0;i<numscripts;i++)
	{
		quad string_length = f->read32le();
		std::string id(string_length,0);
		f->fread(&id[0],string_length);
		f->fgetc(); //null terminator, there for no reason
		
		quad length = f->read32le();
		ScriptBlob* blob = new ScriptBlob();
		blob->data.resize(length);
		f->fread(&blob->data[0],length);
