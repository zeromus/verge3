...
extern VergeCallback renderfunc, timerfunc, zonefunc, realRenderFunc;
...
struct VergeCallback
{
	...
	bool IsValid() const { return functionIndex != -1; }
};
...
public:
	...
	virtual void LoadMapScript(EMUFILE *f, CStringRef filename) = 0;
	...
	void HookZone(VergeCallback s);
	void HookRetrace(VergeCallback s);
	void HookRender(VergeCallback s);
	void HookShowPage(VergeCallback s);
	...
	static void EntityWarp(int e, int x, int y);
