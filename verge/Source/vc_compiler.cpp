...
void Chunk::LoadChunk(EMUFILE *f)
{
	int size;
	fread_le(&size, f);
	chunk.resize(size);
	f->fread(&chunk[0], size);
}
...
void Chunk::SaveChunk(EMUFILE *f)
{
	int size = chunk.size();
	int tempsize = size;
	f->fwrite(&tempsize, 4);
	f->fwrite(&chunk[0], size);
}
...
global_var_t::global_var_t(EMUFILE *f)
{
	memset(&ext, 0, sizeof (ext));

	f->fread(name, IDENTIFIER_LEN);
	f->fread(&type, 1);
	fread_le(&ofs, f);
	fread_le(&len, f);
	fread_le(&dim, f);
...
void global_var_t::write(EMUFILE *f)
{
	f->fwrite(name, IDENTIFIER_LEN);
	f->fwrite(&type, 1);
	f->fwrite(&ofs, 4);
	f->fwrite(&len, 4);
	f->fwrite(&dim, 4);
...
struct_element::struct_element(EMUFILE *f)
{
	memset(&ext, 0, sizeof (ext));

	f->fread(&type, 1);
	f->fread(name, IDENTIFIER_LEN);
	f->fread(type_name, IDENTIFIER_LEN);
	fread_le(&len, f);
	fread_le(&dim, f);
	dims.resize(dim);
	for (int i = 0; i < dim; i++)
...
void struct_element::write(EMUFILE *f)
{
	f->fwrite(&type, 1);
	f->fwrite(name, IDENTIFIER_LEN);
	f->fwrite(type_name, IDENTIFIER_LEN);
	f->fwrite(&len, 4);
	f->fwrite(&dim, 4);
	for (int i = 0; i < dim; i++)
	{
...
struct_definition::struct_definition(EMUFILE *f)
{
	f->fread(name, IDENTIFIER_LEN);
	int element_count;
...
void struct_definition::write(EMUFILE *f)
{
	f->fwrite(name, IDENTIFIER_LEN);
	int element_count = elements.size();
	f->fwrite(&element_count, 4);
	for (int i = 0; i < element_count; i++)
	{
		elements[i]->write(f);
	}
...
struct_instance::struct_instance(EMUFILE *f)
{
	f->fread(name, IDENTIFIER_LEN);
	fread_le(&dim,f);
	dims.resize(dim);
...
void struct_instance::write(EMUFILE *f)
{
	f->fwrite(name, IDENTIFIER_LEN);
	f->fwrite(&dim, 4);
	dims.resize(dim);
...
function_t::function_t(EMUFILE *f)
{
	memset(argtype, 0, sizeof (argtype));
	memset(localnames, 0, sizeof (localnames));
	...
	f->fread(&coreimage, 1);
	f->fread(name, IDENTIFIER_LEN);
	f->fread(argtype, numlocals);
	f->fread(localnames, IDENTIFIER_LEN * numlocals);
}
...
void function_t::write(EMUFILE *f)
{
	f->fwrite(&numargs, 4);
	f->fwrite(&numlocals, 4);
	f->fwrite(&signature, 4);
	f->fwrite(&codeofs, 4);
	f->fwrite(&codeend, 4);
	f->fwrite(&coreimage, 1);
	f->fwrite(name, IDENTIFIER_LEN);
	f->fwrite(argtype, numlocals);
	f->fwrite(localnames, IDENTIFIER_LEN*numlocals);
}
...
: lexer_initialized(false)
{
	...
	for (int i=0; i<NUM_HVARS; i++)
		...
		{
			//mbg 12-dec-2011 - this was added to allow nulled out hvars
			hvars.push_back(new HVar("awbuiawdwaawdnawdaw",""));
		}
	vprint("HVARS...\n");
...
bool VCCompiler::CompileMap(const char *f)
{
	...
	if(result) {
		...
		//std::string canonical_path = f;
		//boost::algorithm::replace_all(canonical_path, "/", "_");
		//FILE *mo = fopen(va("mxv\\%s.mxv", normalize_path(canonical_path).c_str()), "wb");
		//if (!mo) err("couldn't open map for writing");
		...
		mo.fwrite(&z, 4);
		mo.fwrite(&output.chunk[0], z);
		
		ScriptDatabase::ScriptBlob* blob = new ScriptDatabase::ScriptBlob();
		std::string script_id = f;
		script_id += ".map";
		boost::algorithm::to_lower(script_id);
		g_ScriptDatabase.scripts[script_id] = blob;
		blob->data.resize(mo.size());
		memcpy(&blob->data[0],mo.buf(),mo.size());
	}
...
bool VCCompiler::CompileOther(char *f, int cimage, bool append, bool dups, std::vector<int>ref_cimages)
{
	...
	// write if successful
	if(result) {
		EMUFILE_FILE out(va("%s.out", f), "wb");
		//TODO WTF
		...
		int tempz = z;
		out.fwrite(&tempz, 4);
		out.fwrite(&output.chunk[previous_size], z);
	}
...
bool VCCompiler::Process(char *fn, bool required)
{
	int  i;
	char *buf;
	...
	while (*s)
	{
		...
		if (*s == '#')						// crap, its a directive
		{
			...
			//while
			bool optInclude = !strcmp(command, "optinclude");
			if (!strcmp(command, "include") || optInclude)
			{
				if (!*s)
				{
				...
				bool r = Process(temp, !optInclude);		// Preprocess new file
				if (!r)
				{
					delete[] buf;
					pp_included_files.pop_back();
