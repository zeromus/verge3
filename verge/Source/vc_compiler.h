...
public:
	...
	global_var_t();
	global_var_t(EMUFILE *f);
	~global_var_t();
	void write(EMUFILE *f);
};
...
public:
	...
	Chunk();
	~Chunk();
	void LoadChunk(EMUFILE *f);
	void SaveChunk(EMUFILE *f);
...
public:
    ...
    bool Process(char *fn, bool required=true);
    void Init_pptbl();
...
private:
    ...
    void ParseStructMemberDecl(struct_definition* mystruct);
